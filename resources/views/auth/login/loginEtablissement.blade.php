                    ...
                    <div class="xp-auth-box">
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                @endif
                                    <form action="{{route('handleloginetablissement')}}" method="POST">
                                        @method('post')
                                        @csrf
                                        <div class="text-center mb-3">
                                            <h4 class="text-black">Connexion Etablissement !</h4>
                                            {{-- @foreach($etud as $etu)
                                                <p>{{$etu->name}} , {{$etu->email}} ,{{$etu->password}}</p>
                                            @endforeach --}}
                                            <p class="text-muted">Nouveau sur StageBridge? <a href="{{route('registeretablissement')}}">S'inscrire</a> Ici</p>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            <input type="email" name="email" class="form-control" id="username" placeholder="email: exemple@exxemple.com" value="{{ old('email') }}" required>
                                            @error('email')
                                                <div id="a" class="a">
                                                    {{$message}}
                                                </div>
